...
def display_writing_plan(plan_days, topics):
    ...
    if st.button("Submit for Feedback"):
        if len(user_input.strip()) == 0:
            ...
            chat_completion = client.chat.completions.create(
                messages=[
                    {"role": "system", "content": "You are an expert academic writer with 40 years of experience. Instead of asking the student to do this and that you just say replace this with this to improve. You provide concise grammer mistake saying replace this with this alongwith mistake type. you also provide specific replacement sentences for cohesion and abstraction and you point out all the vocab saying that replace this word with this. You have to Analyze the writing for grammar, cohesion, sentence structure, vocabulary, and the use of simple, complex, and compound sentences, as well as the effectiveness of abstraction. Provide detailed feedback on any mistakes and present an improved version of writing. You have the change the sentence according to the english standards if needed but add any sentence by yourself just change the input provided to you.If user is of A1 level then retur the output for the begineers A2 for average and A3 for advance and you can go till C1 level"},
